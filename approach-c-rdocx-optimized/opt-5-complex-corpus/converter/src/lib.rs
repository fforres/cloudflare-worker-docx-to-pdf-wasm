//! opt-5: rdocx-based DOCX → PDF converter that avoids Carlito/Caladea
//! (which trigger a ToUnicode-CMap drift bug in rdocx-pdf 0.1.2).
//!
//! Strategy: bundle only Liberation Sans/Serif/Mono and register them as
//! aliases for every common Microsoft Office font (Calibri, Calibri Light,
//! Cambria, Arial, Times New Roman, Tahoma, Verdana, Georgia, Courier New,
//! Consolas, etc.). Then call `doc.to_pdf_with_fonts(...)`. This produces
//! clean text on both native and wasm32 — matching the rdocx CLI's output
//! on systems where Liberation/system fonts win the lookup.
